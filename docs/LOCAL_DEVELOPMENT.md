...
### Configure environment variables for the local replica

Next.js automatically loads `.env.local`, so copy the provided template and fill it with the IDs generated by DFX:

```shell
cp .env.local.example .env.local
```

- `GYOTAKU_IC_HOST` should remain `http://127.0.0.1:8080` when using `dfx start`.
- After running `make local_deploy`, grab the newly deployed canister ID with `dfx canister id taggr` and place it into `GYOTAKU_CANISTER_ID`.
- The bucket template and Taggr domain can stay on their defaults unless you run a custom bucket canister.
...
Once the prerequisites above are satisfied, you can let a helper script drive the usual `dfx` + `make` commands:

```shell
./scripts/run-local.sh
```

This script checks for `.env.local`, starts the replica (if port 8080 is free), and runs `make local_deploy`, `make dev_build`, and `make local_reinstall`. After it finishes, use `npm start` (or `npm run dev`) in another terminal to launch the frontend.
